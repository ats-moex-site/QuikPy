...
def get_all_accounts():
    ...
    money_limits = qp_provider.GetMoneyLimits()[
        "data"
    ]  # Все денежные лимиты (остатки на счетах)
    depo_limits = qp_provider.GetAllDepoLimits()[
        "data"
    ...
    for trade_account in trade_accounts:  # Пробегаемся по всем счетам
        ...
        distinct_client_code = list(
            set(
                [
                    moneyLimit["client_code"]
        ...
        intersection_class_codes = list(
            set(trade_account_class_codes).intersection(class_codes_list)
        ...
        for class_code in intersection_class_codes:  # Пробегаемся по всем общим классам
            class_info = qp_provider.GetClassInfo(class_code)[
                "data"
            ]  # Информация о классе
            print(
                f'- Класс {class_code} ({class_info["name"]}), Тикеров {class_info["nsecs"]}'
            )
        ...
        if firm_id == futures_firm_id:  # Для фьючерсов свои расчеты
            ...
            # Позиции
            futures_holdings = qp_provider.GetFuturesHoldings()[
                "data"
            ]  # Все фьючерсные позиции
            active_futures_holdings = [
                futuresHolding
                for futuresHolding in futures_holdings
            ...
            for (
                firm_money_limit
            ) in firm_money_limits:  # Пробегаемся по всем денежным лимитам
                ...
                ) in firm_kind_depo_limits:  # Пробегаемся по всем позициям
                    sec_code = firm_kind_depo_limit["sec_code"]  # Код тикера
                    class_code = qp_provider.GetSecurityClass(class_codes, sec_code)[
                        "data"
                    ]
                    entry_price = float(firm_kind_depo_limit["wa_position_price"])
                    last_price = float(
                        qp_provider.GetParamEx(class_code, sec_code, "LAST")["data"][
                            "param_value"
                        ]
...
def get_account(
    ...
    print(
        f"Код клиента {client_code}, Фирма {firm_id}, Счет {trade_account_id}, T{limit_kind}, {currency_code}"
    )
    if futures:  # Для фьючерсов свои расчеты
        ...
        account_money_limit = [
            money_limit
            for money_limit in money_limits  # Денежный лимит
            if money_limit["client_code"] == client_code
            and money_limit["firmid"] == firm_id  # Выбираем по коду клиента
            and money_limit["limit_kind"] == limit_kind  # Фирме
            and money_limit["currcode"] == currency_code  # Дню лимита
        ][
            0
        ]  # Валюте
        ...
        account_depo_limits = [
            depo_limit
            for depo_limit in depo_limits  # Бумажный лимит
            if depo_limit["client_code"] == client_code
            and depo_limit["firmid"] == firm_id  # Выбираем по коду клиента
            and depo_limit["limit_kind"] == limit_kind  # Фирме
            and depo_limit["currentbal"] != 0  # Дню лимита
        ]  # Берем только открытые позиции по фирме и дню
    ...
    account_orders = [
        order
        for order in orders  # Заявки
        if (order["client_code"] == client_code or client_code == "")
        and order["firmid"] == firm_id  # Выбираем по коду клиента
        and order["account"] == trade_account_id  # Фирме
        and order["flags"] & 0b1 == 0b1  # Счету
    ]  # Активные заявки
    for account_order in account_orders:  # Пробегаемся по всем заявкам
        buy = account_order["flags"] & 0b100 != 0b100  # Заявка на покупку
        print(
            f'- Заявка номер {account_order["order_num"]} {"Покупка" if buy else "Продажа"} {account_order["class_code"]}.{account_order["sec_code"]} {account_order["qty"]} @ {account_order["price"]}'
        )
    account_stop_orders = [
        stop_order
        for stop_order in stop_orders  # Стоп заявки
        if (stop_order["client_code"] == client_code or client_code == "")
        and stop_order["firmid"] == firm_id  # Выбираем по коду клиента
        and stop_order["account"] == trade_account_id  # Фирме
        and stop_order["flags"] & 0b1 == 0b1  # Счету
    ...
    for account_stop_order in account_stop_orders:  # Пробегаемся по всем стоп заявкам
        ...
        print(
            f'- Стоп заявка номер {account_stop_order["order_num"]} {"Покупка" if buy else "Продажа"} {account_stop_order["class_code"]}.{account_stop_order["sec_code"]} {account_stop_order["qty"]} @ {account_stop_order["price"]}'
        )
