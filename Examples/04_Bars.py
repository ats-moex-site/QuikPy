...
def save_candles_to_file(
    ...
    for sec_code in sec_codes:  # Пробегаемся по всем тикерам
        ...
        print(
            f"Получение истории {class_code}.{sec_code} {time_frame}{compression} из QUIK"
        )
        new_bars = qp_provider.GetCandlesFromDataSource(
            class_code, sec_code, interval, 0
        )[
            "data"
        ]  # Получаем все бары из QUIK
        pd_bars = pd.json_normalize(
            new_bars
        ...
        pd_bars["datetime"] = pd.to_datetime(
            pd_bars[["year", "month", "day", "hour", "minute", "second"]]
        ...
        pd_bars.volume = pd.to_numeric(
            pd_bars.volume, downcast="integer"
        )  # Объемы могут быть только целыми
        if (
            not file_exists and skip_first_date
        ):  # Если файла нет, и убираем бары на первую дату
            len_with_first_date = len(
                pd_bars
            )  # Кол-во баров до удаления на первую дату
            first_date = pd_bars.index[0].date()  # Первая дата
            pd_bars.drop(
                pd_bars[(pd_bars.index.date == first_date)].index, inplace=True
            )  # Удаляем их
            print(
                f"- Удалено баров на первую дату {first_date}: {len_with_first_date - len(pd_bars)}"
            )
        if skip_last_date:  # Если убираем бары на последнюю дату
            len_with_last_date = len(
                pd_bars
            )  # Кол-во баров до удаления на последнюю дату
            last_date = pd_bars.index[-1].date()  # Последняя дата
            pd_bars.drop(
                pd_bars[(pd_bars.index.date == last_date)].index, inplace=True
            )  # Удаляем их
            print(
                f"- Удалено баров на последнюю дату {last_date}: {len_with_last_date - len(pd_bars)}"
            )
        ...
        if file_exists:  # Если файл существует
            pd_bars = (
                pd.concat([file_bars, pd_bars])
                .drop_duplicates(keep="last")
                .sort_index()
            )  # Объединяем файл с данными из QUIK, убираем дубликаты, сортируем заново
        pd_bars = pd_bars[
            ["open", "high", "low", "close", "volume"]
        ]  # Отбираем нужные колонки. Колонка дата и время будет экспортирована как индекс
