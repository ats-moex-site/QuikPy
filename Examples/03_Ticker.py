...
if __name__ == "__main__":  # Точка входа при запуске этого скрипта
    ...
    # Данные тикера
    si = qp_provider.GetSecurityInfo(classCode, secCode)[
        "data"
    ]  # Интерпретатор языка Lua, Таблица 4.21 Инструменты
    # print('Ответ от сервера:', si)
    print(
        f'Информация о тикере {classCode}.{secCode} ({si["short_name"]}):'
    )  # Короткое наименование инструмента
    tradeAccount = qp_provider.GetTradeAccount(classCode)[
        "data"
    ]  # Торговый счет для класса тикера
    print("Торговый счет:", tradeAccount)
    lastPrice = float(
        qp_provider.GetParamEx(classCode, secCode, "LAST")["data"]["param_value"]
    ...
    print(
        "Цифр после запятой:", si["scale"]
    )  # Точность (кол-во значащих цифр после запятой)
