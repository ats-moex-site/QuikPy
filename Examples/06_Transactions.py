...
if __name__ == "__main__":  # Точка входа при запуске этого скрипта
    ...
    qp_provider.OnFuturesClientHolding = (
        on_futures_client_holding  # Изменение позиции по срочному рынку
    )
    qp_provider.OnDepoLimit = on_depo_limit  # Изменение позиции по инструментам
    qp_provider.OnDepoLimitDelete = (
        on_depo_limit_delete  # Удаление позиции по инструментам
    )
    ...
    slippage = (
        float(
            qp_provider.GetSecurityInfo(class_code, sec_code)["data"]["min_price_step"]
        )
        * StopSteps
    ...
    print(
        f'Новая стоп заявка отправлена на рынок: {qp_provider.SendTransaction(transaction)["data"]}'
    )
