...
if __name__ == "__main__":  # Точка входа при запуске этого скрипта
    ...
    print(
        f'Терминал QUIK подключен к серверу: {qp_provider.IsConnected()["data"] == 1}'
    )
    print(
        f'Отклик QUIK на команду Ping: {qp_provider.Ping()["data"]}'
    ...
    dt = datetime.strptime(
        f"{trade_date} {server_time}", "%d.%m.%Y %H:%M:%S"
    ...
    qp_provider.OnConnected = lambda data: print(
        data
    )  # Нажимаем кнопку "Установить соединение" в QUIK
    qp_provider.OnDisconnected = lambda data: print(
        data
    )  # Нажимаем кнопку "Разорвать соединение" в QUIK
    qp_provider.OnParam = lambda data: print(
        data
