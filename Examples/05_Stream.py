...
if __name__ == "__main__":  # Точка входа при запуске этого скрипта
    ...
    print(
        f"Подписка на изменения стакана {class_code}.{sec_code}:",
        qp_provider.SubscribeLevel2Quotes(class_code, sec_code)["data"],
    )
    print(
        "Статус подписки:",
        qp_provider.IsSubscribedLevel2Quotes(class_code, sec_code)["data"],
    )
    ...
    print(
        f"Отмена подписки на изменения стакана:",
        qp_provider.UnsubscribeLevel2Quotes(class_code, sec_code)["data"],
    )
    print(
        "Статус подписки:",
        qp_provider.IsSubscribedLevel2Quotes(class_code, sec_code)["data"],
    ...
    qp_provider.OnConnected = (
        changed_connection  # Нажимаем кнопку "Установить соединение" в QUIK
    )
    qp_provider.OnDisconnected = (
        changed_connection  # Нажимаем кнопку "Разорвать соединение" в QUIK
    )
    ...
    for interval in (60,):  # (1, 60, 1440) = Минутки, часовки, дневки
        print(
            ...
            qp_provider.SubscribeToCandles(class_code, sec_code, interval)["data"],
        )
        print(
            f"Статус подписки на интервал {interval}:",
            qp_provider.IsSubscribed(class_code, sec_code, interval)["data"],
        )
    input("Enter - отмена\n")
    for interval in (60,):  # (1, 60, 1440) = Минутки, часовки, дневки
        print(
            f"Отмена подписки на интервал {interval}",
            qp_provider.UnsubscribeFromCandles(class_code, sec_code, interval)["data"],
        )
        print(
            f"Статус подписки на интервал {interval}:",
            qp_provider.IsSubscribed(class_code, sec_code, interval)["data"],
